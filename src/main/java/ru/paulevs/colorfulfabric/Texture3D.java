...
public class Texture3D {
	...
	private void makeEmpty() {
		...
		//GL20.glTexSubImage3D(GL20.GL_TEXTURE_3D, 0, 0, 0, 0, SIDE, SIDE, SIDE, GL20.GL_RGB8, GL20.GL_UNSIGNED_BYTE, buffer);
	}
	...
	public Texture3D fillTexture(byte[] red, byte[] green, byte[] blue) {
		...
		for (int i = 0; i < red.length; i++) {
			...
			buffer.put(blue[i]);
			buffer.flip();
			GL20.glTexSubImage3D(GL20.GL_TEXTURE_3D, 0, x, y, z, 1, 1, 1, GL20.GL_RGB, GL20.GL_UNSIGNED_BYTE, buffer);
		}
